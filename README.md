...
Fixed-Time (Webster) — cycle length and split computed from Webster's delay-minimizing formula given the regime's effective saturation flow.
Vanilla Max-Pressure — full-strength phase-pressure control with no downstream awareness (Varaiya, 2013), included as the baseline this work is designed to improve upon.
Fuzzy Anti-Spillback Max-Pressure — the proposed controller (§3).
...
Statistically significant delay reduction relative to unmodified Max-Pressure (§5.1).
Substantially improved fairness relative to fixed-time, without paying Max-Pressure's full aggregate-delay cost.
A mechanistically explained, direction-localized cost (§5.4) rather than an unexplained one.
A measurable, causally isolated benefit to emergency-vehicle transit time when preemption is enabled (§5.5).
